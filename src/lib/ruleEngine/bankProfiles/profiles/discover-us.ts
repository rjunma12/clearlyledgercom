/**
 * Discover Bank US Profile
 * Credit card + banking
 */

import type { BankProfile } from '../types';

export const discoverUsProfile: BankProfile = {
  id: 'discover-us',
  name: 'Discover Bank',
  region: 'US',
  defaultLocale: 'en-US',
  identification: {
    logoPatterns: ['DISCOVER', 'DISCOVER BANK', 'discover.com', 'DISCOVER CARD'],
    accountPatterns: [/\b\d{10,16}\b/],
    uniqueIdentifiers: ['DOABORUS', 'DOAB'],
    confidenceThreshold: 0.7,
  },
  columnConfig: {
    columnOrder: 'date-desc-amount-balance',
    mergedDebitCredit: true,
    debitIndicators: ['-', 'DR', 'Debit'],
    creditIndicators: ['+', 'CR', 'Credit'],
    balancePosition: 'right',
    hasReferenceColumn: false,
  },
  specialRules: {
    dateFormatting: {
      dateFormats: ['MM/DD/YYYY', 'MM-DD-YYYY', 'MMM DD, YYYY'],
      dateSeparator: '/',
      yearFormat: '4-digit',
    },
    amountFormatting: {
      currencySymbol: '$',
      symbolPosition: 'prefix',
      negativeFormat: 'minus',
    },
    skipPatterns: [
      /^account\s+number/i,
      /^statement\s+period/i,
      /^page\s+\d+/i,
      /^credit\s+limit/i,
    ],
    openingBalancePatterns: [
      /^previous\s+balance/i,
      /^beginning\s+balance/i,
    ],
    closingBalancePatterns: [
      /^new\s+balance/i,
      /^ending\s+balance/i,
    ],
    multiLineDescriptions: true,
    maxDescriptionLines: 2,
  },
  version: '1.0.0',
  lastUpdated: '2025-02-07',
};
